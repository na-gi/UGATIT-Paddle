...
class UGATIT(object):
    def __init__(self, args):
        ...
        # res = fluid.get_flags('FLAGS_cudnn_deterministic')
        # if res and self.benchmark_flag:
        #     print('set benchmark!')
    ...
    def build_model(self):
        ...
        self.trainA = ImageFolder(os.path.join('data', self.dataset, 'trainA'), train_transform)
        self.trainB = ImageFolder(os.path.join('data', self.dataset, 'trainB'), train_transform)
        self.testA = ImageFolder(os.path.join('data', self.dataset, 'testA'), test_transform)
        self.testB = ImageFolder(os.path.join('data', self.dataset, 'testB'), test_transform)

        self.trainA_loader = paddle.batch(paddle.reader.shuffle(myreader(self.trainA), 30000),batch_size=self.batch_size)
        self.trainB_loader = paddle.batch(paddle.reader.shuffle(myreader(self.trainB), 30000),batch_size=self.batch_size)
        self.testA_loader = paddle.batch(paddle.reader.shuffle(myreader(self.testA), 30000),batch_size=self.batch_size)
        self.testB_loader = paddle.batch(paddle.reader.shuffle(myreader(self.testB), 30000),batch_size=self.batch_size)

        tmp = next(self.trainA_loader())
        print('一个batch图片数据的形状：batch_size =', len(tmp), ', data_shape =', tmp[0].shape)
    ...
    def train(self):
        ...
        with fluid.dygraph.guard(place):
            ...
            for step in range(start_iter, self.iteration + 1):
                ...
                real_A = fluid.dygraph.to_variable(real_A[0])
                real_B = fluid.dygraph.to_variable(real_B[0])
    ...
    def sload(self):
        pa='results/selfie2anime/smodel/genA2B_1000.pdparams'
        a,c=fluid.load_dygraph(pa)
        
        pb='results/selfie2anime/smodel/genB2A_1000.pdparams'
        b,h=fluid.load_dygraph(pb)
        
        # pc='results/selfie2anime/smodel/G_optim_1000.pdopt'
        # _,c=fluid.load_dygraph(pc)
        
        pd='results/selfie2anime/smodel/disGA_1000.pdparams'
        d,_=fluid.load_dygraph(pd)
        
        pe='results/selfie2anime/smodel/disGB_1000.pdparams'
        e,_=fluid.load_dygraph(pe)
        
        pf='results/selfie2anime/smodel/disLA_1000.pdparams'
        f,_=fluid.load_dygraph(pf)   
        
        pg='results/selfie2anime/smodel/disLB_1000.pdparams'
        g,_=fluid.load_dygraph(pg)
        
        # ph='results/selfie2anime/smodel/D_optim_1000.pdopt'
    ...
    def test(self):
        ...
        with fluid.dygraph.guard(place):
            ...
            self.test_load(os.path.join(self.result_dir, self.dataset, 'model'), 1000)
